...
struct SetupStatusTests {
    ...
    @Test func anUnreadableKeyboardListIsNotTreatedAsNotInstalled() {
        #expect(InstalledKeyboards.includesLocalFlow(nil) == nil)
    ...
    @Test func theKeyboardListMatchesEntriesCarryingLayoutOptions() {
        let entries = [
            ...
            "com.example.localflow.keyboard@sw=QWERTY;hw=Automatic",
        ]

        #expect(InstalledKeyboards.includesLocalFlow(entries) == true)
        #expect(InstalledKeyboards.includesLocalFlow(["emoji@sw=Emoji"]) == false)
    }
    ...
    @Test func severalOutstandingStepsAreCountedRatherThanNamed() {
        ...
        #expect(status.attentionHeadline == "Local Flow needs 2 more steps")
    }
