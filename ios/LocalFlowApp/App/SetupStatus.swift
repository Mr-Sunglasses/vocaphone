...
enum InstalledKeyboards {
    ...
    static func includesLocalFlow() -> Bool? {
        includesLocalFlow(UserDefaults.standard.array(forKey: preferenceKey) as? [String])
    }

    /// Entries carry trailing layout options, so the bundle identifier is
    /// matched as a substring rather than compared whole.
    static func includesLocalFlow(_ identifiers: [String]?) -> Bool? {
...
struct SetupStatus: Equatable, Sendable {
    ...
    var attentionHeadline: String? {
        ...
        guard blockingSteps.count == 1 else {
            return "Local Flow needs \(blockingSteps.count) more steps"
        }
        switch first {
        case .gateway:
    ...
    func detail(for step: SetupStep) -> String {
        ...
        case .microphone:
            switch microphone {
            case .granted:
                return "Local Flow records on this iPhone; the keyboard only "
            ...
            case .denied:
                return "Microphone access was declined. Turn it back on for "
                    + "Local Flow in iOS Settings › Privacy & Security › Microphone."
            }
        case .keyboard:
            switch keyboard {
            case .notAdded:
                return "In iOS Settings, open General › Keyboard › Keyboards › "
                    + "Add New Keyboard and choose Local Flow, then tap it again "
                    + "and turn on Allow Full Access."
            case .addedButNeverRun:
                return "Local Flow is in your keyboard list. Turn on Allow Full "
                    + "Access for it, then switch to it in the field below — "
                    + "this ticks itself the moment it runs."
            case .seenWithoutFullAccess:
                return "Local Flow is added, but Full Access is off. Tap Local "
